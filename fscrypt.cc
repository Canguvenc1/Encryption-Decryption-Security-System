...
void my_cbc_encrypt(const unsigned char *inn, unsigned char *out, long length,
	const BF_KEY *schedule, unsigned char *ivec, int enc)
{
	...
	if (enc == BF_DECRYPT)			//cbc decrypt algorithm
	{
		while (length > 0)
		{
			...
			for (int i = 0; i < BLOCKSIZE; i++)
			{
				out[i] = (unsigned char)(out[i] ^ ivec[i]);
				ivec[i] = temp[i];
			}
			inn += BLOCKSIZE;
			out += BLOCKSIZE;
	...
	else						//cbc encrypt algorithm
	{
		while (length > 0)
		{
			for (int i = 0; i < BLOCKSIZE; i ++)
			{
				out[i] = (unsigned char)(inn[i] ^ ivec[i]);
			}
...
void *fs_encrypt(void *plaintext, int bufsize, char *keystr, int *resultlen)
{
	...
	my_cbc_encrypt(inn, result, *resultlen, key_str, init_vec, BF_ENCRYPT);
	return (void *)result;
}
...
void *fs_decrypt(void *ciphertext, int bufsize, char *keystr, int *resultlen)
{
	...
	for (int i = 0; i < bufsize; i++)
		result[i] = 0;
	my_cbc_encrypt((unsigned char *)ciphertext, result, bufsize, key_str, init_vec, BF_DECRYPT);
	int i;
